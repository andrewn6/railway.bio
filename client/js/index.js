...
// prolly need to update the html its just place holder lol
const renderTrainDepartures = (trainDepartures) => {
  const trainDeparturesElement = $('#train-departures');
  trainDeparturesElement.innerHTML = '';
  ...
  trainDepartures.forEach((departure) => {
    ...
    departureElement.innerHTML = `
        <div class="time">${departure.scheduledTime}</div>
        <div class="service">${departure.service}</div>
        <div class="platform">${departure.platform || '-'}</div>
        <div class="stops">${departure.stopsDisplay}</div>
        <div class="info">${departure.info || ''}</div>
      `;
    trainDeparturesElement.appendChild(departureElement);
  });
}

const updateTrainDepartures = async () => {
  const trainDepartures = await fetchTrainDepartures();
  renderTrainDepartures(trainDepartures);
};

setInterval(updateTrainDepartures, 30000);
